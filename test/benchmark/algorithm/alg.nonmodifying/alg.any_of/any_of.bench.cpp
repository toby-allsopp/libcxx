...
void BM_any_of(benchmark::State& st) {
    ...
    while (st.KeepRunning()) {
        DoNotOptimize(std::any_of(test_arr.begin(), test_arr.end(), &is_neg1));
        DoNotOptimize(std::any_of(test_arr2.begin(), test_arr2.end(), &is_neg1));
        DoNotOptimize(test_arr);
        DoNotOptimize(test_arr2);
    }
}
BENCHMARK(BM_any_of)->Arg(1<<14);
