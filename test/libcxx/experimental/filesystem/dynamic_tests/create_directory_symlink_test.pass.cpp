...
TEST_SUITE(elib_filesystem_dynamic_create_directory_symlink_test_suite)
...
TEST_CASE(dne_no_error_code_test)
{
    ...
    // with error code
    TEST_REQUIRE_NO_THROW(create_directory_symlink(file, to));
    TEST_REQUIRE(is_symlink(to));
}
...
TEST_CASE(already_exists_test)
{
    ...
    {
        TEST_REQUIRE_THROW(filesystem_error, create_directory_symlink(file, to));
    }
}
