...
void test_const_get_if() {
    ...
    }
    {
        ...
        ASSERT_SAME_TYPE(decltype(std::get_if<int>(std::addressof(v))), int const*);
        static_assert(*std::get_if<int>(std::addressof(v)) == 42, "");
        static_assert(std::get_if<long>(std::addressof(v)) == nullptr, "");
    }
    {
        using V = std::variant<int, long>;
        constexpr V v(42l);
        ASSERT_SAME_TYPE(decltype(std::get_if<long>(std::addressof(v))), long const*);
        static_assert(*std::get_if<long>(std::addressof(v)) == 42, "");
        static_assert(std::get_if<int>(std::addressof(v)) == nullptr, "");
    }
    {
        using V = std::variant<int&>;
        int x = 42;
