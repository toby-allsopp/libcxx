...
BENCHMARK_CAPTURE(BM_ContainerInsertValue,
    ...
    std::unordered_set<uint32_t>{}, getSortedIntegerInputs<uint32_t>)->Arg(TestNumInputs);

BENCHMARK_CAPTURE(BM_ContainerInsertValue,
    unordered_set_uint32_larger_than_bucket_count,
    std::unordered_set<uint32_t>{}, getSortedLargeIntegerInputs<uint32_t>)->Arg(TestNumInputs);
